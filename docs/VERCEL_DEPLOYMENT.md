...
## 6) Build behavior note

- Project is configured so **lint is skipped during Vercel build** (`next.config.ts -> eslint.ignoreDuringBuilds = true`).
- Run `npm run lint` in CI or locally as a separate quality gate.
